...
* A `track` is a collection of data recorded by the device and sent to the server, according to the device configuration. The structure of a track on the wire is optimized to limit bandwith usage. Use tracks to collect device speed, position, battery level, or other car data. The complete description of the tracking sysem is out of the scope of this documentation.
...
An order is an event sent to your agent when it needs to perform a task. These orders are sent according to your shceduled orders configuration in `config/scheduled.rb`.
...
Time to write some code. Let's start by something simple (but useless) : an "echo" agent that reply back to the device with any message it receives.
...
2. In `initial.rb`, add code for teh `new_msg_from_device` callback.
...
Look at the documentation for {UserApis::MdiClass}. You will find the list of API under the "namespace" `user_api.mdi`. From here, you can navigate the documentation to find the object you are looking for. Renember that you do not need to instantiate any class in the `UserApi` module, this is done for you.
...
Keep in mind that your agent can be restarted at any time, that its Redis cache could be flushed at any moment, that several instances of your agent could be running at the same time but only one will be received a given message...
